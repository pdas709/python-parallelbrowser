...
def test_Edit_Summary(browser):
    ...
    summary_section_.summaryfield()

    summary_section_.availabilty_check()
    summary_section_.availabilty_check()

    summary_section_.expected_rate()

    summary_section_.clickk_save()

    summary_input_text = summary_section_.verify_summary_text()
    assert initialize.summary_field in summary_input_text, f"Test Failed: Alert did not match expected."  
