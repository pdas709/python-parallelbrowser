...
def test_Education_Null_Validation(browser):
    # Add Item to cart
    ...
    try:
      ...
      raise Exception('Unable to find text in this element after waiting 10 seconds')
    #proceed with edit education and check null input validation
    education_section_ = EducationSection(browser)
    education_section_.click_edit_button()
    education_section_.clear_university_name()
    education_section_.save_educationfield_asitis()

    #Verify The Alert Text
    try:
      WebDriverWait(browser, 10).until(
          EC.visibility_of_element_located((By.XPATH, '//span[contains(.,"required")]')))
    except TimeoutException:
      raise Exception('Unable to find text in this element after waiting 10 seconds')

    txt_from_allerrt = education_section_.get_alerttt_text_and_verify()
    assert 'required' in txt_from_allerrt, f"Test Failed: Input did not match expected - required"  
